...
func main() {
	...
	isContractor := role == Contractor && (today == Friday || today == Saturday || today == Sunday)
	isMember := role == Member && weekday(today)
	isGuest := role == Guest && (today == Monday || today == Wednesday || today == Friday)

	if isAdmin || isManager || isContractor || isMember || isGuest {
		accessGranted()
	} else {
		accessDenied()
	}
